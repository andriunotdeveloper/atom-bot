...
class Starboard {
	...
	add(message, starredBy) {
		...
		if (blacklist.includes(starredBy.id)) {
			return '<:disagree:745003854017593354> You have been blacklisted from using the starboard';
		}

		if (!this.channel) {
			const prefix = this.client.commandHandler.prefix(message);
			return `<:disagree:745003854017593354> There isn't a starboard channel to use. Set one using the \`${prefix}starboard\` command!`;
		}
		...
		if (message.author.id === starredBy.id) {
			if (!this.recentlySelfstarred.has(message.author.id)) {
				...
				return '<:disagree:745003854017593354> You can\'t star your own messages.';
			}
	...
	async addStar(message, starredBy) {
		...
		if (star.starredBy.includes(starredBy.id)) {
			return '<:disagree:745003854017593354> You have already starred this message before; You can\'t star it again.';
		}
	...
	missingPermissions() {
		...
		return `<:disagree:745003854017593354> 'm missing ${str} to do that!`;
	}
