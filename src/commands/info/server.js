...
class ServerInfoCommand extends Command {
	...
	async exec(message) {
		...
		const embed = this.client.util.embed().setColor('invisible')
			...
			.addField('❯ Channels Info', [
				...
				`• AFK: ${message.guild.afkChannelID ? `<#${message.guild.afkChannelID}> after ${message.guild.afkTimeout / 60} min` : '**N/A**'}`
			])
			.addField('❯ Other', [
