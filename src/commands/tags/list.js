...
class TagListCommand extends Command {
	...
	async exec(message, { member }) {
		if (member) {
			...
			if (!tags.length) {
				if (member.id === message.author.id) return message.util.channel.send('<:disagree:745003854017593354> You don\'t have any tags.');
				return message.util.channel.send(`<:disagree:745003854017593354> **${member.displayName}** doesn't have any tags.`);
			}
		...
		const embed = new MessageEmbed().setColor("invisible")
