...
class PlaylistAddCommand extends Command {
	...
	async exec(message, { playlist, query }) {
		if (playlist.userID !== message.author.id) return message.util.channel.send('<:disagree:745003854017593354> You can only add songs to your own playlists.');
		...
		return message.util.send(`<:agree:745003896522670100> **Added to playlist:** \`${msg}\``);
	}
}
