...
class PlayCommand extends Command {
	...
	async exec(message, { query, unshift }) {
		...
		if (['TRACK_LOADED', 'SEARCH_RESULT'].includes(res.loadType)) {
			...
			return message.util.channel.send('<:disagree:745003854017593354> I couldn\'t find what you were looking for.');
		}
		if (!queue.player.playing && !queue.player.paused) await queue.start();

		return message.util.channel.send(`${this.client.emojis.get('745003896522670100')} **Queued up:** \`${msg}\``);
	}
}
