...
class BlacklistCommand extends Command {
	...
	async exec(message, { member }) {
		...
		if (blacklist.includes(member.id)) {
			...
			return message.util.send(`<:agree:745003896522670100> **${member.user.tag}** has been removed from the blacklist.`);
		}
		blacklist.push(member.id);
		await this.client.settings.set(message.guild, 'blacklist', blacklist);

		return message.util.send(`<:agree:745003896522670100> **${member.user.tag}** has been blacklisted from using the starboard and giving reputation on this server.`);
	}
}
