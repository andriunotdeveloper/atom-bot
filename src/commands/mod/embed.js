...
class RestrictEmbedCommand extends Command {
	...
	async exec(message, { member, reason }) {
		...
		if (member.roles.has(staffRole)) {
			return message.channel.send('You know you can\'t do this.');
		}

		const restrictRoles = this.client.settings.get(message.guild, 'restrictRoles', undefined);
		if (!restrictRoles) return message.channel.end('<:disagree:745003854017593354> There are no restricted roles configured on this server.');
		...
		try {
			...
			return message.channel.send(`<:disagree:745003854017593354> There was an error embed retricting this member: \`${error}\``);
		}
		...
		return message.util.send(`<:agree:745003896522670100> Successfully embed restricted **${member.user.tag}**`);
	}
}
