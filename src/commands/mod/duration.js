...
class DurationCommand extends Command {
	...
	async exec(message, { caseNum, duration }) {
		...
		if (isNaN(caseToFind)) return message.channel.send('At least provide me with a correct number.');
		...
		if (modLogChannel) {
			...
			if (!caseEmbed) return message.channel.send('Looks like the message doesn\'t exist anymore!');
		...
		return message.util.send(`<:agree:745003896522670100> Successfully updated duration for case **#${caseToFind}**`);
	}
}
