...
class RestrictEmojiCommand extends Command {
	...
	async exec(message, { member, reason }) {
		...
		if (member.roles.has(staffRole)) {
			return message.channel.send('<:disagree:745003854017593354> You know you can\'t do this.');
		}

		const restrictRoles = this.client.settings.get(message.guild, 'restrictRoles', undefined);
		if (!restrictRoles) return message.channel.send('<:disagree:745003854017593354> There are no restricted roles configured on this server.');

		const key = `${message.guild.id}:${member.id}:EMOJI`;
		if (this.client.cached.has(key)) {
			return message.channel.send('<:disagree:745003854017593354> That user is currently being moderated by someone else.');
		}
		...
		try {
			...
			return message.channel.send(`<:disagree:745003854017593354> There was an error emoji restricting this member: \`${error}\``);
		}
		...
		return message.util.send(`<:agree:745003896522670100> Successfully emoji restricted **${member.user.tag}**`);
	}
}
