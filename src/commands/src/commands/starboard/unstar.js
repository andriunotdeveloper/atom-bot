	...
	*args() {
	...
	async exec(message, { message: msg }) {
		if (msg.author.id === message.author.id) {
			return message.util.channel.send('<:disagree:745003854017593354> You can\'t unstar your own message.');
		}
		...
		if (!message.channel.permissionsFor(this.client.user).has('MANAGE_MESSAGES')) {
			return message.util.channel.send('<:disagree:745003854017593354> I\'m missing `Manage Messages` to unstar that message in this channel.');
		}

		const missingPerms = starboard.missingPermissions();
		if (missingPerms) return message.util.channel.send(missingPerms);

		const star = await Star.findOne({ where: { messageID: msg.id } });

		if (!star || !star.starredBy.includes(message.author.id)) {
			return message.util.channel.send('<:disagree:745003854017593354> You can\'t remove any star from this message because you never gave it one in the first place.');
		}

		const error = await starboard.remove(msg, message.author);
		if (error) return message.util.channel.send(error);
		return message.util.channel.send('<:agree:745003896522670100> The message has been unstarred.');
	}
}
