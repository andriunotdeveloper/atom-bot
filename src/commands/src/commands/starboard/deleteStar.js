	...
	*args() {
	...
	async exec(message, { message: msg }) {
		...
		if (!await Star.findOne({ where: { messageID: msg.id } })) {
			return message.util.channel.send('<:disagree:745003854017593354> The message cannot be removed because it does not exist in the starboard.');
		}
		...
		return message.util.channel.send('<:agree:745003896522670100> The message has been removed from the starboard.');
	}
}
