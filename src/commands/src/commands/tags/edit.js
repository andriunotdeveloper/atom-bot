...
class TagEditCommand extends Command {
	...
	async exec(message, { tag, hoist, unhoist, content }) {
		...
		if (tag.authorID !== message.author.id && !staffRole) {
			return message.util.channel.send('<:disagree:745003854017593354> Losers are only allowed to edit their own tags!');
		}
		...
		return message.util.channel.send(`<:agree:745003896522670100> Successfully edited **${tag.name}**${hoist && staffRole ? ' to be hoisted.' : '.'}`);
	}
}
