	...
	*args() {
	...
	async exec(message, { first, second, add, del }) {
		...
		if (add) {
			...
			return message.util.channel.send('<:disagree:745003854017593354> You have to either supply `--add` or `--del`');
		}

		return message.util.channel.send(`<:agree:745003896522670100> Alias **${second.substring(0, 256)}** ${add ? 'added to' : 'deleted from'} tag **${first.name}**.`);
	}
}
